...
describe('isTeamSupportedBackend', () => {
  it.each(['claude', 'codex', 'opencode', 'gemini', 'hermes'])('allows verified backend "%s"', (backend) => {
    expect(isTeamSupportedBackend(backend)).toBe(true);
  });

  it.each(['aionrs', 'openclaw-gateway', 'nanobot', 'remote', 'qwen', 'copilot', 'kimi', 'goose', 'codebuddy'])(
...
describe('filterTeamSupportedAgents', () => {
  ...
  it('uses presetAgentType over backend when available', () => {
    // presetAgentType 'qwen' is unverified even though the host backend 'claude' is verified
  ...
  it('returns empty array when no agents are supported', () => {
    const agents = [makeAgent('aionrs'), makeAgent('remote'), makeAgent('qwen')];
...
describe('MCP injection chain consistency', () => {
  ...
  it('TEAM_SUPPORTED_BACKENDS contains the full verified set', () => {
    // Verified backends must each resolve to a MCP-capable conversationType.
    // See the `every verified backend must resolve to a MCP_CAPABLE_TYPE` test above.
    expect([...TEAM_SUPPORTED_BACKENDS].toSorted()).toEqual(['claude', 'codex', 'gemini', 'hermes', 'opencode']);
  });
