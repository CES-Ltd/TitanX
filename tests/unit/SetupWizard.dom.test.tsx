...
describe('SetupWizard', () => {
  ...
  it('renders the three mode cards on the first screen', () => {
    ...
    expect(screen.getByText('fleet.wizard.title')).toBeInTheDocument();
    expect(screen.getAllByText('fleet.mode.regular.name')[0]).toBeInTheDocument();
    expect(screen.getAllByText('fleet.mode.master.name')[0]).toBeInTheDocument();
    expect(screen.getAllByText('fleet.mode.slave.name')[0]).toBeInTheDocument();
  });

  it('disables Continue until a mode is picked', () => {
    render(<SetupWizard visible onComplete={noop} />);
    const continueBtn = screen.getByRole('button', { name: /fleet.wizard\.continue/ });
    ...
    expect(screen.getByRole('button', { name: /fleet.wizard\.continue/ })).not.toBeDisabled();
  });

  it('Regular mode skips the configure step and goes straight to "done"', async () => {
    render(<SetupWizard visible onComplete={noop} />);
    fireEvent.click(screen.getByRole('button', { name: 'fleet.mode.regular.name' }));
    fireEvent.click(screen.getByRole('button', { name: /fleet.wizard\.continue/ }));
    await waitFor(() => {
      expect(screen.getByText('fleet.wizard.done')).toBeInTheDocument();
      expect(screen.getByRole('button', { name: /fleet.wizard\.launch/ })).toBeInTheDocument();
    });
  });
  ...
  it('calls onComplete when Launch clicked from "done"', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'fleet.mode.regular.name' }));
    fireEvent.click(screen.getByRole('button', { name: /fleet.wizard\.continue/ }));
    await waitFor(() => screen.getByText('fleet.wizard.done'));
    fireEvent.click(screen.getByRole('button', { name: /fleet.wizard\.launch/ }));
  ...
  it('"skip" (cancel) calls completeSetup with regular then onComplete', async () => {
    const onComplete = vi.fn();
    render(<SetupWizard visible onComplete={onComplete} />);
    fireEvent.click(screen.getByRole('button', { name: /fleet.wizard\.cancel/ }));
    await waitFor(() => {
  ...
  it('Master picker → configure screen shows port field and bind radios', () => {
    ...
    expect(screen.getByText('fleet.wizard.master.portLabel')).toBeInTheDocument();
  });
  ...
  it('Slave picker → configure screen shows URL + token inputs', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /fleet.wizard\.continue/ }));
    expect(screen.getByText('fleet.wizard.slave.title')).toBeInTheDocument();
    expect(screen.getByText('fleet.wizard.slave.urlLabel')).toBeInTheDocument();
    expect(screen.getByText('fleet.wizard.slave.tokenLabel')).toBeInTheDocument();
  });
