...
export function initCostTrackingBridge(): void {
  ipcBridge.costTracking.record.provider(async (input) => {
    console.log(
      `[CostBridge] record: provider=${input.provider} model=${input.model} input=${String(input.inputTokens)} output=${String(input.outputTokens)}`
    );
  ...
  ipcBridge.costTracking.summary.provider(async ({ userId, fromDate }) => {
    ...
    console.log(
      `[CostBridge] summary: events=${String(result.eventCount)} input=${String(result.totalInputTokens)} output=${String(result.totalOutputTokens)} cost=${String(result.totalCostCents)}c`
    );
    return result;
  });
