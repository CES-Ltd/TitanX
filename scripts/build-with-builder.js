...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('TitanX.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running TitanX/Electron process. Attempting to close...');
        killWindowsProcesses(['TitanX.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running TitanX/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'TitanX.exe');
    ...
    console.log('⚠️  Windows local build failed after TitanX.exe was produced.');
    if (firstError) {
